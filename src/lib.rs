...
fn create_pulsar(env: Env, options: Option<PulsarOptions>) -> External<Arc<Pulsar<TokioExecutor>>> {
  let addr_from_js = options.clone().map(|o| o.url).flatten().or_else(|| env::var("ADDON_PULSAR_BINARY_URL").ok()).unwrap_or_else(|| "pulsar://127.0.0.1:6650".to_string());
  let token_from_js = options.clone().map(|o| o.token).flatten().or_else(|| env::var("ADDON_PULSAR_TOKEN").ok());
...
#[napi]
fn create_pulsar_producer(pulsar: External<Arc<Pulsar<TokioExecutor>>>, options: Option<PulsarProducerOptions>) -> External<Arc<Mutex<Producer<TokioExecutor>>>> {
  let pulsar_arc = Arc::clone(pulsar.as_ref());

  let topic_from_js = options.clone().map(|o| o.topic).flatten().or_else(|| env::var("ADDON_PULSAR_TOPIC").ok()).unwrap_or_else(|| "non-persistent://public/default/test".to_string());
  ...
  #[napi]
  fn send_pulsar_message(producer: External<Arc<Mutex<Producer<TokioExecutor>>>> , options: Option<PulsarMessageOptions>) -> Null {
